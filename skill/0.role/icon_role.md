...
- **Primary Source**: `icon/miniDev-icon/操作图标/` (Operation Icons)
- **File Type**: SVG
- **Version**: v1.1
...
- **Primary Location**: 大多数功能性图标 (Search, Close, Delete, Settings etc.) 位于 `icon/miniDev-icon/操作图标/`。
- **Naming Note**: 文件名多为中文或“中文(english)”格式。
    - Search -> `icon/miniDev-icon/操作图标/搜索.svg`
    - Close -> `icon/miniDev-icon/操作图标/close.svg` 或 `icon/miniDev-icon/操作图标/清除.svg`
    - Settings -> `icon/miniDev-icon/操作图标/设置.svg`
    - Check -> `icon/miniDev-icon/操作图标/已选.svg`
...
<!-- 注意：路径需根据实际文件结构调整 -->
<img src="icon/miniDev-icon/操作图标/搜索.svg" class="devui-icon-search" alt="search" />
...
.devui-input-icon {
  ...
  height: 16px;
  /*引用本地路径*/
  background-image: url('icon/miniDev-icon/操作图标/搜索.svg');
